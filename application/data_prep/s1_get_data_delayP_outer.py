...
path_data = f"data/s2_filter_trial/delayprice_filter_trial.csv"
path_subinfo = f"data/s3_subjinfo/delayprice_filter_trial_subjinfo.csv"

df_data = pd.read_csv(path_data)
df_subinfo = pd.read_csv(path_subinfo)

path_data = f"data/s2_filter_trial/delaychoice_filter_trial.csv"
path_subinfo = f"data/s3_subjinfo/delaychoice_filter_trial_subjinfo.csv"
df_data_outer = pd.read_csv(path_data)
df_subinfo_outer = pd.read_csv(path_subinfo)

df_data = df_data[df_data["filter_trial"] == 1].reset_index(drop=True)
df_data_outer = df_data_outer[df_data_outer["filter_trial"] == 1].reset_index(drop=True)

df_data_outer["response"] = (df_data_outer["response"] == 1).astype(int)
df_data_outer["response2"] = ((df_data_outer.a_money > df_data_outer.b_money).astype(int) == df_data_outer.response).astype(int)

tmp = df_subinfo[(df_subinfo["filter_subj"] == 1) & (df_subinfo["n_trials"] >30)]["subj_num"].unique()
tmp_outer = df_subinfo_outer[(df_subinfo_outer["filter_subj"] == 1) & (df_subinfo_outer["n_trials"] >15)]["subj_num"].unique()

all_subj = np.array(list(set(tmp) & set(tmp_outer)))
...
df_data_outer = df_data_outer[df_data_outer["subj_num"].isin(all_subj)]
...
col_norm_outer = ['a_money', 'a_days', 'b_money', 'b_days']
...
for subj in all_subj:
    ...
    df_subj_outer = df_data_outer[df_data_outer["subj_num"] == subj]
...
for idx_subj in range(n_subj):
    ...
    tmp = df_subj[["money", "time", "response"]].values
...
data_list_all_outer = []
for idx_subj in range(n_subj):
    df_subj = df_data_outer[df_data_outer["subj_num"] == all_subj[idx_subj]]
    tmp = df_subj[['a_money', 'a_days', 'b_money', 'b_days', "response"]].values

    tmp = tmp[np.random.permutation(len(tmp))]
    data_list_all_outer.append(tmp)
