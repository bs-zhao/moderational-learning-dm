...
data_name = ["Flex6Lv2", "Flex6Lv2b", "Hete4", "Hete4bn4"][1]
...
draw_sim1_recovery(all_Y_real[:20], all_Y_pred[:20], mus_test, norm=norm, max_factors=8,
                   width_ratios=[1.5, 1, 1,1], wspace=0.55,
                   axis_label_size=14, tick_label_size=12, title_size=14, suptitle_size=16,
                   suptitle_text=f"Number of latent factors: {input_dim}",
                   supersuper="(b)")
...
fig, axes = plt.subplots(1, nc, figsize=(14, 3.2))

for i in range(nc):

    axes[i].scatter(X_c[:, i], Y_c[:, i], alpha=0.7, color="#C44E52")

    axes[i].set_title(f"Component {i+1}", fontsize=14)
    ...
    axes[i].set_xlabel("θ Projection", fontsize=14)
    if i == 0:
        axes[i].set_ylabel("μ Projection", fontsize=14)
...
for i in range(nc):
    ...
    axes[i].scatter(
        y, yh,
        alpha=0.6, color="#C44E52", s=25, linewidths=0
    )
...
if "Hete" in data_name:
    ...
    plt.tight_layout()
    fig.text(0.00, 1.10, "(b)", fontsize=11, fontweight="bold",
             ha="left", va="top")
    ...
    plt.tight_layout()
    fig.text(0.00, 1.10, "(c)", fontsize=11, fontweight="bold",
             ha="left", va="top")
    fig.text(0.52, 1.10, "(d)", fontsize=11, fontweight="bold",
             ha="left", va="top")
    plt.show()
...
if "Hete" in data_name:
    ...
    fig, axes = plt.subplots(1, nc, figsize=(5, 2.5))
    ...
    for i in range(nc):
        ...
        if i<3:
            ...
            print(model.summary())
            yh = model.predict(x)
            axes[i].scatter(y, yh, alpha=0.6, color="#4C72B0", s=25, linewidths=0)
            axes[i].set_xlabel("True", fontsize=11)
            if i == 0:
                axes[i].set_ylabel("Predicted", fontsize=11)
            axes[i].set_title(f"θ{i+1}", fontsize=11)
            r = np.corrcoef(y, yh)[0, 1]
            axes[i].text(0.95, 0.05, f"$r$ = {r:.3f}",
                         transform=axes[i].transAxes,
                         ha="right", va="bottom",
    ...
    for i in range(nc):
        ...
        if i<3:
            if i == 0:
                axes[i].set_ylabel("Density", fontsize=11)
            axes[i].hist(Y_sorted[:,i], bins=35, alpha=0.7, color="#55A868", density=1)
            axes[i].set_title(f"μ{i+1}", fontsize=11)
            axes[i].set_xlabel("Value", fontsize=11)
            if i == 3:
                axes[i].set_ylabel("Density", fontsize=11)
...
draw_sim1_recovery(all_Y_real[:20], all_Y_pred[:20], mus_test, norm=norm, max_factors=8,
                   width_ratios=[1.5, 1, 1,1], wspace=0.4,
                   axis_label_size=11, tick_label_size=11, title_size=11, suptitle_size=11,
                   suptitle_text=f"Number of latent factors: {input_dim}",supersuper = " "
                   )
